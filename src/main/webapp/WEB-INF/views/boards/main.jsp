...
<div class="container">
	...
	<table class="table table-striped">
		...
		<tbody>
			...
			<c:forEach var="MainDto" items="${MainDtos}">
				<tr>
					<td>${MainDto.id}</td>
					<td><a href="/boards/${MainDto.id}">${MainDto.title}</a></td>
					<td>${MainDto.username}</td>
				</tr>
			</c:forEach>
	...
	<div class="d-flex justify-content-center">
		<ul class="pagination">
			...
			<c:forEach var="num" begin="${pagingDto.startPageNum}" end="${pagingDto.lastPageNum}" step="1">
				<li class='page-item ${pagingDto.currentPage == num-1 ? "active" : ""}'>
				<a
